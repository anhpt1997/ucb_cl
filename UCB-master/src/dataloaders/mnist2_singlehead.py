...
def get(data_path,seed,fixed_order=False,pc_valid=0):
    ...
    # Validation
    for t in data.keys():
        data[t]['valid'] , data[t]['valid_train']={} , {}
        data[t]['valid_train']['x'] , data[t]['valid']['x'], data[t]['valid_train']['y'] , data[t]['valid']['y'] = train_test_split(data[t]['train']['x'].clone() , data[t]['train']['y'].clone() , test_size = 0.1 , random_state = 2)
