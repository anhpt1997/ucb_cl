# -*- coding: utf-8 -*-

import os,sys
import numpy as np
import torch
from torchvision import datasets,transforms
import pickle

########################################################################################################################

def get(data_path,seed,fixed_order=False,pc_valid=0):
    
    file_name = "../data/NotMNIST/train_test_small_notmnist"
    data={}
    taskcla=[]
    size=[1,28,28]

    # MNIST
    mean=(0.1307,)
    std=(0.3081,)
    dat={}
#    dat['train']=datasets.MNIST(data_path,train=True,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
#    dat['test']=datasets.MNIST(data_path,train=False,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
    data_all_notmnist = pickle.load(open(file_name,"rb"))
    dat['train'] = tuple(zip(data_all_notmnist['image_train'].astype(np.float32),data_all_notmnist['label_train'].astype(np.float32)))
    dat['test']= tuple(zip(data_all_notmnist['image_test'].astype(np.float32), data_all_notmnist['label_test'].astype(np.float32)))
#    print("type data train ", type(dat['train']))
    data[0]={}
    data[0]['name']='mnist-0-1'
    data[0]['ncla']=10
    data[1]={}
    data[1]['name']='mnist-2-3'
    data[1]['ncla']=10
    data[2]={}
    data[2]['name']='mnist-4-5'
    data[2]['ncla']=10
    data[3]={}
    data[3]['name']='mnist-6-7'
    data[3]['ncla']=10
    data[4]={}
    data[4]['name']='mnist-8-9'
    data[4]['ncla']=10
    for s in ['train','test']:
        loader=torch.utils.data.DataLoader(dat[s],batch_size=1,shuffle=False)
        print("loader ",loader)
        data[0][s]={'x': [],'y': []}
        data[1][s]={'x': [],'y': []}
        data[2][s]={'x': [],'y': []}
        data[3][s]={'x': [],'y': []}
        data[4][s]={'x': [],'y': []}
        for image,target in loader:
            
#            print("type image", type(image),"shape ", image.shape)
            label=target.numpy()
            if label==0:
                data[0][s]['x'].append(image)
                data[0][s]['y'].append(label)
            if label==1:
                data[0][s]['x'].append(image)
                data[0][s]['y'].append(label)

            if label==2:
                data[1][s]['x'].append(image)
                data[1][s]['y'].append(label)
            if label==3:
                data[1][s]['x'].append(image)
                data[1][s]['y'].append(label)

            if label==4:
                data[2][s]['x'].append(image)
                data[2][s]['y'].append(label)
            if label==5:
                data[2][s]['x'].append(image)
                data[2][s]['y'].append(label)

            if label==6:
                data[3][s]['x'].append(image)
                data[3][s]['y'].append(label)
            if label==7:
                data[3][s]['x'].append(image)
                data[3][s]['y'].append(label)

            if label==8:
                data[4][s]['x'].append(image)
                data[4][s]['y'].append(label)
            if label==9:
                data[4][s]['x'].append(image)
                data[4][s]['y'].append(label)


    # "Unify" and save
    for n in range(5):
        for s in ['train','test']:
            data[n][s]['x']=torch.stack(data[n][s]['x']).view(-1,size[0],size[1],size[2])
            data[n][s]['y']=torch.LongTensor(np.array(data[n][s]['y'],dtype=int)).view(-1)

    # Validation
    for t in data.keys():
        data[t]['valid']={}
        data[t]['valid']['x']=data[t]['train']['x'].clone()
        data[t]['valid']['y']=data[t]['train']['y'].clone()

    # Others
    n=0
    for t in data.keys():
        taskcla.append((t,data[t]['ncla']))
        n+=data[t]['ncla']
    data['ncla']=n

    return data,taskcla,size

########################################################################################################################
