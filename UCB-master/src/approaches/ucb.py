...
class Appr(object):
	...
	def train(self,t,xtrain,ytrain,xvalid,yvalid):
		...
		try:
			for e in range(self.nepochs):
				...
				if e % 1 == 0:
					valid_loss,valid_acc=self.eval(t,xvalid[-1],yvalid[-1])
					print('Epoch ',e,' Valid: loss={:.3f}, acc={:5.1f}% |'.format(valid_loss, 100 * valid_acc), end='')
	...
	def elbo_loss(self, input, target, t, num_batches, sample,debug=False):
		if sample:
			...
			return (log_var)/num_batches + nll
