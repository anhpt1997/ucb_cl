...
class Appr(object):
    ...
    def eval_list_valid(self, t , list_xvalid , list_yvalid):
        ...
        for i in range(len(list_xvalid)):
            loss ,acc =self.test(i , list_xvalid[i] , list_yvalid[i])
        ...
        with torch.no_grad():
            ...
            for i in range(0,len(r),self.sbatch):
                ...
                for i in range(num_sample):
                    output=self.model(images,sample=False)[: , (5*t):end ].cpu().numpy()
                    loss = self.elbo_loss(images, targets, t, num_batches,sample=False)
                    list_output.append(output)
                    list_loss.append(loss.cpu().numpy())

                output_mean = np.mean( np.array(list_output), axis = 0)
                loss_mean = np.mean(np.array(list_loss))
                pred=np.argmax(output_mean, axis = 1)+5*t
    ...
    def elbo_loss(self, input, target, t, num_batches, sample,debug=False):
        if sample:
            ...
            for i in range(self.samples):
                predictions.append(self.model(input,sample=sample))
                lp, lv = self.logs(0)
                lps.append(lp)
                lvps.append(lv)
            ...
            predictions = []
            for i in range(self.samples):
                pred = self.model(input,sample=False)
            ...
            outputs = torch.stack(predictions,dim=0).to(self.device)
            nll = w3*torch.nn.functional.nll_loss(outputs.mean(0), target, reduction='mean').to(device=self.device)
