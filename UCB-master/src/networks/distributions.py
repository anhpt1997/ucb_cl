...
class GaussMixtureVariationalPosterior(torch.nn.Module):
    ...
    def sample(self):
        ...
        gumbel_softmax = self.sample_gumbel_softmax().to(self.device)
        gauss_sample  =self.sample_gauss().to(self.device)
        return torch.sum(torch.mul(gumbel_softmax , gauss_sample) , dim= 0 ).to(self.device)
    ...
    def sample_test(self):
        gumbel_softmax = self.sample_gumbel_softmax().to(self.device)
        return torch.sum(torch.mul(gumbel_softmax , self.mu) , dim= 0 ).to(self.device)
